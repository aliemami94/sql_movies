...
WITH FilmPerformance AS (
    ...
    SELECT
        ...
        -- Count of unique customers who rented this film
    ...
    GROUP BY
        f.film_id, f.title, f.rental_duration, f.length, c.name
),
...
SELECT
    fp.title,
    fp.category_name,
    -- Use a CASE statement to categorize films by their length
    CASE
        WHEN fp.length <= 60 THEN 'Short'
        WHEN fp.length > 60 AND fp.length <= 120 THEN 'Medium'
        ELSE 'Long'
    END AS film_length_category,
    fp.total_revenue,
    -- Calculate the percentage of this film's revenue out of the total revenue for all films
    (fp.total_revenue / SUM(fp.total_revenue) OVER ()) * 100 AS percentage_of_total_revenue,
    fp.rental_count,
    fp.unique_renters,
    fp.avg_rental_days,
    fp.may_2005_rentals,
